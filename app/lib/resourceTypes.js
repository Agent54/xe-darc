export default {
    ...
    'navigate-off-origin': {
        ...
        description: 'Navigate to pages on different domains',
        defaultMockBehavior: 'lightbox',
        availability: async () => {
            return { available: true, error: null }
        }
    ...
    media: {
        ...
        availability: async () => {
            if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
                return { available: false, error: 'Media API not supported by browser' }
            }
            return { available: true, error: null }
        }
    },
    ...
    'clipboard-read': {
        ...
        icon: `<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" d="M15.666 3.888A2.25 2.25 0 0 0 13.5 2.25h-3c-1.03 0-1.9.693-2.166 1.638m7.332 0c.055.194.084.4.084.612v0a.75.75 0 0 1-.75.75H9a.75.75 0 0 1-.75-.75v0c0-.212.03-.418.084-.612m7.332 0c.646.049 1.288.11 1.927.184 1.1.128 1.907 1.077 1.907 2.185V19.5a2.25 2.25 0 0 1-2.25 2.25H6.75A2.25 2.25 0 0 1 4.5 19.5V6.257c0-1.108.806-2.057 1.907-2.185a48.208 48.208 0 0 1 1.927-.184" />
            <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" opacity="0.6" />
            <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" opacity="0.6" />
        </svg>`,
        description: 'Clipboard read-only access',
        availability: async () => {
            if (!navigator.clipboard) {
                return { available: false, error: 'Clipboard API not supported by browser' }
            }
            try {
                const permission = await navigator.permissions.query({ name: 'clipboard-read' })
                if (permission.state === 'denied') {
                    return { available: false, error: 'Clipboard read permission denied by user' }
                }
                return { available: true, error: null }
            } catch (e) {
                return { available: false, error: null }
    ...
    'clipboard-write': {
        ...
        icon: `<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" d="M15.666 3.888A2.25 2.25 0 0 0 13.5 2.25h-3c-1.03 0-1.9.693-2.166 1.638m7.332 0c.055.194.084.4.084.612v0a.75.75 0 0 1-.75.75H9a.75.75 0 0 1-.75-.75v0c0-.212.03-.418.084-.612m7.332 0c.646.049 1.288.11 1.927.184 1.1.128 1.907 1.077 1.907 2.185V19.5a2.25 2.25 0 0 1-2.25 2.25H6.75A2.25 2.25 0 0 1 4.5 19.5V6.257c0-1.108.806-2.057 1.907-2.185a48.208 48.208 0 0 1 1.927-.184" />
            <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125" opacity="0.6" />
        </svg>`,
        description: 'Clipboard write-only access',
        availability: async () => {
            if (!navigator.clipboard) {
                return { available: false, error: 'Clipboard API not supported by browser' }
            }
    ...
    clipboardReadWrite: {
        ...
        description: 'Full clipboard read and write access',
        availability: async () => {
            if (!navigator.clipboard) {
                return { available: false, error: 'Clipboard API not supported by browser' }
            }
