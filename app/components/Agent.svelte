<script>
    ...
    let availableTargets = $derived.by(() => {
        ...
        const currentSpaceName = data.spaceMeta.activeSpace ? (data.spaces[data.spaceMeta.activeSpace]?.name || 'Unnamed Space') : 'No active space'
        ...
        return [
            ...
            },
            {
                ...
                type: 'current-space',
                description: data.spaceMeta.activeSpace ? `${data.spaces[data.spaceMeta.activeSpace]?.tabs?.length || 0} tabs` : 'No active space'
            },
            {
                id: 'all-spaces',
    ...
    async function processMessage(queueItem) {
        ...
        try {
            ...
            const cardTypes = [
                { type: 'tool', action: 'web-search', status: 'accessed', title: 'Web Search', details: 'Searching for relevant information' },
                { type: 'tool', action: 'execute-js', status: 'accessed', title: 'JavaScript Execution', details: currentTab ? currentTab.title || 'Current Tab' : 'No target tab' },
                { type: 'tool', action: 'screenshot', status: 'accessed', title: 'Page Analysis', details: 'Analyzing current page content' },
                { type: 'tool', action: 'modify-content', status: 'modified', title: 'Content Modification', details: 'Modifying page content' },
                { type: 'security', action: 'camera-access', status: 'accessed', title: 'Camera Access', details: 'Requesting camera permission' },
                { type: 'security', action: 'microphone-access', status: 'denied', title: 'Microphone Access', details: 'User denied microphone access' },
                { type: 'tool', action: 'clipboard-access', status: 'accessed', title: 'Clipboard Access', details: 'Reading/writing clipboard' },
                { type: 'security', action: 'location-access', status: 'accessed', title: 'Location Access', details: 'Accessing device location' },
                { type: 'tool', action: 'file-download', status: 'accessed', title: 'File Download', details: 'Downloading file to device' },
                { type: 'security', action: 'ad-block', status: 'denied', title: 'Ad Blocked', details: 'Blocked advertisement content' },
                { type: 'tool', action: 'api-call', status: 'accessed', title: 'API Request', details: 'Making external API call' },
                { type: 'security', action: 'analytics-block', status: 'denied', title: 'Analytics Blocked', details: 'Blocked tracking scripts' }
            ]
                    ...
                    {#if message.role === 'info-cards'}
                        <div class="agent-info-cards">
                            {#each message.cards as card}
                                <div class="agent-info-card" class:context={card.type === 'context'} class:tool={card.type === 'tool'} class:security={card.type === 'security'} class:accessed={card.status === 'accessed'} class:modified={card.status === 'modified'} class:denied={card.status === 'denied'}>
                                    <div class="agent-info-card-header">
                                        <div class="agent-info-card-icon">
                                            ...
                                            {:else if card.action === 'proxy-detection'}
                                                <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" d="M9.348 14.652a3.75 3.75 0 010-5.304m5.304 0a3.75 3.75 0 010 5.304m-7.425 2.121a6.75 6.75 0 010-9.546m9.546 0a6.75 6.75 0 010 9.546M5.106 18.894c-3.808-3.808-3.808-9.98 0-13.789m13.788 0c3.808 3.808 3.808 9.981 0 13.79M12 12h.008v.007H12V12zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z" />
                                                </svg>
                                            ...
                                            {:else if card.action === 'print-dialog'}
                                                <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" d="M6.72 13.829c-.24.03-.48.062-.72.096m.72-.096a42.415 42.415 0 0 1 10.56 0m-10.56 0L6.34 18m10.94-4.171c.24.03.48.062.72.096m-.72-.096L17.66 18m0 0 .229 2.523a1.125 1.125 0 0 1-1.12 1.227H7.231c-.662 0-1.18-.568-1.12-1.227L6.34 18m11.318 0h1.091A2.25 2.25 0 0 0 21 15.75V9.456c0-1.081-.768-2.015-1.837-2.175a48.055 48.055 0 0 0-1.913-.247M18.75 9V6a2.25 2.25 0 0 0-2.25-2.25H7.5A2.25 2.25 0 0 0 5.25 6v3M6.75 9h10.5m-10.5 0V6a2.25 2.25 0 0 1 2.25-2.25h6A2.25 2.25 0 0 1 17.25 6v3" />
                                                </svg>
                                            {/if}
                                        </div>
